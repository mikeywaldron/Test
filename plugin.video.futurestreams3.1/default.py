...
def stream_video(params):
    ...
    url = get_live("aHR0cDovL2Z1bmt5Y2hpY2tlbi50azoyMDUyL2VuaWdtYTIucGhwP3VzZXJuYW1lPSVzJnBhc3N3b3JkPSVzJnR5cGU9Z2V0X2xpdmVfc3RyZWFtcyZjYXRfaWQ9JXM=")%(kasutajanimi,salasona,CatID)
    ...
    for channel in tree.findall(sync_data("Y2hhbm5lbA==")): #channel
        ...
        poo = striimilink
        if "http://funkychicken.tk:2052/"  in striimilink: 
...
def run_cronjob(params):
    ...
    if "http://"  not in lopplink: 
        lopplink = get_live("aHR0cDovL2Z1bmt5Y2hpY2tlbi50azoyMDUyL2xpdmUvJXMvJXMvJXM=")%(kasutajanimi,salasona,lopplink)
        lopplink = lopplink[:-2]
...
def execute_ainfo(params):
    ...
    #plugintools.add_item( action="",   title=vod_channels("U2lnbiB1cCBhdCBGdXR1cmVTdHJlYW1zLnRr") , thumbnail=os.path.join(LOAD_LIVE,vod_channels("bXlhY2MucG5n")) , fanart=os.path.join(LOAD_LIVE,sync_data("YmFja2dyb3VuZC5wbmc=")) , folder=False )
